...
public class SysOracleCreateTableManagerImpl implements SysOracleCreateTableManager{
	...
	public void createOracleTable(){
		...
		Map<String, Object> oracleTypeAndLengthMap = oracleTypeAndLengthMap();
		...
		// 构建出全部表的增删改的map
		allTableMapConstruct(oracleTypeAndLengthMap, classes, newTableMap, modifyTableMap, addTableMap, removeTableMap, dropKeyTableMap,dropUniqueTableMap);
	...
	private void allTableMapConstruct(
			Map<String, Object> oracleTypeAndLengthMap,
			Set<Class<?>> classes,
		...
		for (Class<?> clas : classes){
			...
			tableFieldsConstruct(oracleTypeAndLengthMap, clas, newFieldList);
	...
	/**
	 * 获取Oracle的类型，以及类型需要设置几个长度，这里构建成map的样式 构建Map(字段名(小写),需要设置几个长度(0表示不需要设置，1表示需要设置一个，2表示需要设置两个))
	 */
	public Map<String, Object> oracleTypeAndLengthMap(){
		Field[] fields = OracleTypeConstant.class.getDeclaredFields();
		Map<String, Object> map = new HashMap<String, Object>();
		for (Field field : fields){
